    ...
    def load_val_or_test_image_paths(self):
        ...
        for i, video_name in enumerate(self.video_paths):
            ...
            if not os.path.isdir(video_path):
                print(f"视频目录 {video_path} 不存在，跳过该视频样本。")
                continue  # 如果视频目录不存在，则跳过
            ...
            if self.test_adv and self.labels[i] == 1:
                for c in [1, 2, 3, 4, 5]:
                    ...
                    # 检查对抗样本路径是否存在
                    if not os.path.isdir(video_path_adv):
                        print(f"对抗样本目录 {video_path_adv} 不存在，跳过该视频样本。")
                        continue  # 如果对抗样本路径不存在，则跳过
                ...
                try:
                    ...
                    print(f"视频目录 {video_path} 中的文件不足10个，跳过该视频样本。")
                    continue  # 如果视频目录中的文件少于10个，则跳过
    ...
    def __getitem__(self, index):
        ...
        while True:
            ...
            if self.data_type == 'train':
                ...
                try:
                    # 1) 拿到一个随机帧路径
                    image_path, image_name = self.get_image_path(
                    ...
                    if image is None:
                        raise IOError(f"cv2.imread failed for {image_path}")
                    break  # 成功，跳出循环
                except (FileNotFoundError, IOError) as e:
                    attempts += 1
                    if attempts >= max_attempts:
                        # 实在取不到就报错，或返回一个默认／空样本
                        raise e
                    # 否则随机挑一个新的 index 再试
        ...
        # 拿到合法的 image, label，下面接原来的 transform + one_hot
